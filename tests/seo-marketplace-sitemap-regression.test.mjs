...
test('sitemaps are deterministic, sharded and contain clean canonical paths only', () => {
  ...
  assert.match(geoLandingSource, /place\?\.slug && !make && resolveCategoryMake\(category, place\.slug\)/)
  assert.match(geoLandingSource, /place\?\.slug && make && !model && resolveMakeModel\(category, make, place\.slug\)/)
  assert.match(sitemapShardSource, /buildSeoMarketplacePath/)
