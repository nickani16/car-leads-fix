...
const seoMarketplaceSource = readFileSync(new URL('../app/components/SeoMarketplaceLanding.tsx', import.meta.url), 'utf8')
...
test('clean SEO URLs render Marketplace metadata without crawler database work', () => {
  assert.match(seoRouteSource, /import SeoMarketplaceLanding/)
  assert.match(seoRouteSource, /<SeoMarketplaceLanding landing=\{landing\}/)
  assert.match(seoRouteSource, /export const dynamic = 'force-dynamic'/)
  ...
  assert.doesNotMatch(marketplacePageSource, /searchMarketplaceListings/)
  assert.match(marketplacePageSource, /seoLanding\s*\? \[\]/)
  assert.match(seoMarketplaceSource, /listings=\{\[\]\}/)
  assert.doesNotMatch(seoMarketplaceSource, /next\/headers|createAdminClient|searchMarketplaceListings/)
  ...
  assert.match(vehicleSearchSource, /AUTOMATED_CRAWLER_PATTERN\.test\(window\.navigator\.userAgent\)/)
...
test('sitemaps are deterministic, sharded and contain clean canonical paths only', () => {
  ...
  assert.match(geoLandingSource, /collidingMake && getSeoSitemapMakes\(category\)\.includes\(collidingMake\)/)
  assert.match(geoLandingSource, /collidingModel && getSeoSitemapModels\(category\)\.some/)
  assert.match(sitemapShardSource, /buildSeoMarketplacePath/)
  assert.doesNotMatch(sitemapShardSource, /marketplace\?/)
  assert.match(robotsSource, /sitemapIndexUrlsForRequest\(request\)/)
  assert.match(robotsSource, /Sitemap: \$\{sitemap\}/)
