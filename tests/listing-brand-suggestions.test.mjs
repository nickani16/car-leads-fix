...
test('brand suggestions include common vehicle, machine and caravan makers plus typo aliases', () => {
  ...
  assert.match(brandSuggestionSource, /damerauLevenshtein/)
  assert.match(brandSuggestionSource, /brandCorrectionSuggestion/)
...
test('listing create and edit forms wire make suggestions into the make field', () => {
  ...
  assert.match(newListingFormSource, /brandCorrectionSuggestion\(category, values\.make \|\| ''\)/)
  assert.match(newListingFormSource, /<Field[\s\S]*name="make"[\s\S]*suggestions=\{makeSuggestions\}/)
  assert.match(newListingFormSource, /correctionSuggestion=\{makeCorrectionSuggestion\}/)
  ...
  assert.match(editListingFormSource, /brandCorrectionSuggestion\(listing\.category, make\)/)
  assert.match(editListingFormSource, /makeSuggestionsOpen && \(visibleMakeSuggestions\.length \|\| makeCorrectionSuggestion\)/)
  ...
  assert.match(newListingFormSource, /Menade du \$\{suggestion\}\?/)
  assert.match(editListingFormSource, /Menade du \$\{suggestion\}\?/)
})
