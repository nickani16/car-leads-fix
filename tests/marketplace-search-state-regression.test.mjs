...
test('marketplace mode defaults to all while sale and leasing stay query-separated', () => {
  ...
  assert.match(vehicleSearchExperienceSource, /const visibleMeta = layout === 'split' \|\| layout === 'desktopSplit' \? meta\.slice\(0, 2\) : meta/)
...
test('marketplace comparison is localized, capped at four and uses a comparison matrix', () => {
  ...
  assert.match(vehicleSearchExperienceSource, /type ResultsLayout = 'single' \| 'split' \| 'desktopSplit'/)
  assert.match(vehicleSearchExperienceSource, /useState<ResultsLayout>\('split'\)/)
  assert.match(vehicleSearchExperienceSource, /grid min-\[1120px\]:grid-cols-2/)
  assert.match(vehicleSearchExperienceSource, /layout=\{resultsLayout === 'split' && filteredListings\.length > 1 \? 'desktopSplit' : 'single'\}/)
  assert.match(vehicleSearchExperienceSource, /min-\[1120px\]:grid-cols-1 min-\[1120px\]:items-stretch/)
