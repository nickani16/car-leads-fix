...
test('marketplace mode defaults to all while sale and leasing stay query-separated', () => {
  ...
  assert.match(homeHeroVehicleSearchSource, /if \(intent !== 'all'\)/)
  ...
  assert.match(homeHeroVehicleSearchSource, /type Intent = 'all' \| 'sale' \| 'leasing'/)
...
test('marketplace cards and listing detail show stable rounded offer status', () => {
  ...
  assert.match(vehicleSearchExperienceSource, /shouldShowListingCountryChip\(listing\.country, marketCountryCode\)/)
  assert.doesNotMatch(vehicleSearchExperienceSource, /CountryFlag/)
...
test('marketplace comparison is localized, capped at four and uses a comparison matrix', () => {
  ...
  assert.match(vehicleSearchExperienceSource, /resultsLayout === 'split' && filteredListings\.length > 1 \? 'grid grid-cols-2 gap-3 p-3' : ''/)
...
test('marketplace mobile shortcuts use existing map, saved search and sorting controls', () => {
  ...
  assert.doesNotMatch(vehicleSearchExperienceSource, /7\.25rem\)\] lg:hidden/)
...
test('mobile bottom navigation glass avoids white outlines and adapts inactive item contrast', () => {
  ...
  assert.match(publicHeaderSource, /largestViewportHeight/)
  assert.match(publicHeaderSource, /viewportShortByScreen/)
  assert.match(publicHeaderSource, /viewportShortBySession/)
  assert.match(publicHeaderSource, /removeProperty\('--autorell-mobile-bottom-gap'\)/)
...
test('proxy protects expensive crawl surfaces without blocking verified search bots', () => {
  ...
  assert.match(proxySource, /GOOGLE_OTHER_PATTERN = \/googleother\/i/)
  assert.match(proxySource, /X-Autorell-Bot-Protection': 'google-other-blocked'/)
  ...
  assert.doesNotMatch(proxySource, /'\/sitemaps\/:path\*'/)
