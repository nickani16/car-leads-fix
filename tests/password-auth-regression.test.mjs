...
test('password registration uses Supabase Auth without a parallel password store', () => {
  assert.match(signupRoute, /admin\.auth\.admin\.generateLink/)
  assert.match(signupRoute, /type: 'signup'/)
  assert.match(signupRoute, /getSignupConfirmationEmailCopy/)
  assert.match(signupRoute, /new Resend\(resendKey\)\.emails\.send/)
  assert.match(signupRoute, /sessionReady: false/)
  assert.doesNotMatch(signupRoute, /auth\.verifyOtp/)
  assert.match(authCallback, /tokenHash && type === 'signup'/)
  assert.match(authCallback, /token_hash: tokenHash,[\s\S]*type: 'signup'/)
