...
test('public listing images use safe framing and modern formats by viewport', () => {
  ...
  assert.doesNotMatch(processingSource, /background:\s*['"`](#000|black)/i)
  assert.doesNotMatch(processingSource, /\.flatten\(/)
  assert.doesNotMatch(processingSource, /\.extend\(/)
  ...
  assert.doesNotMatch(publicCompanySource, /rounded-\[10px\] bg-black/)
  assert.match(publicCompanySource, /ListingCardImageCarousel/)
  assert.match(publicCompanySource, /showControlsOnDesktop/)
  assert.match(publicCompanySource, /showDotsOnDesktop/)
  assert.doesNotMatch(accountListingsSource, /bg-black md:rounded-l-\[19px\]/)
  assert.match(accountListingsSource, /quality=\{78\} className="object-cover"/)
  assert.doesNotMatch(listingFormSource, /bg-black object-contain/)
})
