...
test('sale and leasing modes stay aligned across URL state, search results and metadata', () => {
  ...
  assert.match(vehicleSearchSource, /router\.replace\(nextUrl, \{ scroll: false \}\)/)
  assert.match(marketplacePageSource, /title: \{ absolute: title \}/)
  assert.doesNotMatch(vehicleSearchSource, /syncDocumentTitle|syncDocumentMeta|new MutationObserver\(syncMetadata\)/)
