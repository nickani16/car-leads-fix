...
function normalizeMarketSelection(values: string[], fallback = '') {
  ...
  const fallbackValue = fallback.toUpperCase()
  return [
    ...new Set(
      normalized.length
        ? normalized
        : fallbackValue && selectableMarketCodes.has(fallbackValue)
          ? [fallbackValue]
          : [],
    ),
  ]
...
function matchesSelectedMarkets(country: string, markets: string[]) {
  return !markets.length || markets.includes(country.toUpperCase())
}
