...
export default function RevealPhoneButton({ listingId, locale }: RevealPhoneButtonProps) {
  ...
  if (visible) {
    return (
      <a
        href={telHref}
        className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[12px] border border-[#d9e1ec] bg-white px-5 text-base font-black text-[#101828] transition hover:border-[#0866ff] hover:text-[#0866ff]"
  ...
  return (
    <button
      ...
      onClick={revealPhone}
      disabled={loading}
      className="inline-flex min-h-12 w-full items-center justify-center rounded-[12px] border border-[#cfd8e6] bg-white px-5 text-base font-black text-[#0866ff] transition hover:border-[#0866ff] hover:bg-[#f5f9ff]"
    >
