...
export default function LeadReviewActions({
  ...
  initialBuyNowPrice,
  initialReservePrice,
  initialSellerTargetPrice,
  initialAutorellPurchasePrice,
}: {
  leadId: string
  status: string | null
  ...
  initialSellerTargetPrice?: number | string | null
  initialAutorellPurchasePrice?: number | string | null
  ...
  const [purchasePrice, setPurchasePrice] = useState(
    initialAutorellPurchasePrice
      ? String(initialAutorellPurchasePrice)
      : initialSellerTargetPrice
        ? String(initialSellerTargetPrice)
        : ''
  )

  if (status !== 'Pending review' && initialAutorellPurchasePrice) return null
  ...
  return (
    <section className="mb-7 rounded-[22px] border border-[#9bc9e4] bg-[#eef7fb] p-6">
      ...
      <label className="mt-5 block max-w-sm">
        ...
        />
        {initialSellerTargetPrice ? (
          <span className="mt-2 block text-xs text-[#617681]">
            Supplier requested €
            {Number(initialSellerTargetPrice).toLocaleString()}.
          </span>
        ) : null}
      </label>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        >
          <span className="flex items-center gap-2 text-sm font-semibold">
            <Gavel size={17} />
            Timed export auction
          </span>
          <span className="mt-2 block text-xs leading-5 text-[#617681]">
            European dealers compete to buy the vehicle from Autorell.
          </span>
        </button>
        <button
          type="button"
        ...
        >
          <span className="flex items-center gap-2 text-sm font-semibold">
            ...
            Fixed Autorell price
          </span>
          <span className="mt-2 block text-xs leading-5 text-[#617681]">
            Autorell offers the vehicle at a fixed export sales price.
          </span>
        </button>
      </div>
      ...
      {saleFormat === 'marketplace' ? (
        <label className="mt-4 block max-w-sm">
          <span className="mb-2 block text-xs font-semibold text-[#617681]">
            Autorell sale price to buyer (EUR)
          </span>
          <input
            type="number"
            min="1"
        ...
        <label className="mt-4 block max-w-sm">
          <span className="mb-2 block text-xs font-semibold text-[#617681]">
            Minimum Autorell sale price / auction reserve (EUR)
          </span>
          <input
            type="number"
            min="1"
            ...
            placeholder="Must protect at least €1,500 or 5%"
            className="h-12 w-full rounded-[14px] border border-[#9bc9e4] bg-white px-4 text-sm outline-none focus:border-[#397b9f] focus:ring-4 focus:ring-[#B4D9EF]/35"
          />
        </label>
      )}
