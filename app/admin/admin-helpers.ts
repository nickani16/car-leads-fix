...
export function statusTone(
  ...
  if (['paused', 'waiting_for_customer', 'review', 'restricted', 'flagged', 'past_due'].includes(status)) {
    return 'amber'
  }
  if (['rejected', 'blocked', 'removed', 'deleted', 'suspended', 'critical', 'high', 'unpaid', 'failed'].includes(status)) {
    return 'red'
  }
  return 'gray'
}
