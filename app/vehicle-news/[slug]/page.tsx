...
export async function generateMetadata({
  ...
  const result = await getVehicleNewsArticle('en', slug, preview)
  if (!result) return {}
  const { article } = result
  return {
    title: article.seoTitle || `${article.title} | Autorell`,
    description: article.metaDescription || article.excerpt,
    alternates: { canonical: article.canonicalUrl || `https://www.autorell.com/vehicle-news/${slug}` },
...
export default async function EnglishVehicleNewsArticlePage({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string }>
  ...
  return (
    <main className="min-h-screen bg-white text-[#101828]">
      <PublicHeader locale="en" marketCode="EU" />
      {result.preview ? (
        <div className="bg-amber-100 px-4 py-3 text-center text-sm font-semibold text-amber-900">
          Secure preview - this page is not indexed.
        </div>
      ) : null}
      <article className="mx-auto max-w-[900px] px-5 py-10 sm:px-8 sm:py-16">
        <nav className="text-sm text-[#667085]" aria-label="Breadcrumb">
          <Link href="/vehicle-news" className="hover:text-[#0866ff]">Vehicle news</Link>
          {article.category ? <> / <span>{article.category.label}</span></> : null}
        </nav>
        {article.category ? <p className="mt-8 text-xs font-semibold uppercase tracking-[.18em] text-[#0866ff]">{article.category.label}</p> : null}
        ...
        <div className="mt-6 flex flex-wrap gap-3 text-sm text-[#667085]">
          ...
          <span>|</span>
          <time>{new Date(article.publishedAt).toLocaleDateString('en-GB')}</time>
          <span>|</span>
      ...
      <NewsletterSignup locale="en" category="vehicle-news" />
