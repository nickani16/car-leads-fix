    ...
    <>
      <form id="bulk-listing-form" onSubmit={requestAction} className="mt-4 flex flex-col gap-3 rounded-[16px] border border-[#cbd8ea] bg-[#f8fbff] p-3 sm:flex-row sm:items-center">
        <label className="inline-flex items-center gap-2 text-sm font-medium text-[#344054]">
          ...
          {text('Välj alla på sidan', 'Select page')}
        </label>
        <span className="text-sm text-[#667085]">{selectedCount} {text('valda', 'selected')}</span>
        <select aria-label={text('Bulkåtgärd', 'Bulk action')} value={action} onChange={(event) => setAction(event.target.value)} className="h-10 rounded-[11px] border border-[#cbd5e1] bg-white px-3 text-sm font-medium outline-none focus:border-[#0866ff] focus:ring-4 focus:ring-[#0866ff]/10 sm:ml-auto">
          <option value="">{text('Välj åtgärd', 'Choose action')}</option>
          <option value="pause">{actionText('pause')}</option>
          <option value="resume">{actionText('resume')}</option>
          <option value="mark_sold">{actionText('mark_sold')}</option>
          <option value="delete">{actionText('delete')}</option>
        </select>
        <button type="submit" disabled={!selectedCount || !action} className="h-10 rounded-[11px] bg-[#101828] px-4 text-sm font-semibold text-white outline-none transition hover:bg-[#0866ff] focus-visible:ring-4 focus-visible:ring-[#0866ff]/25 disabled:opacity-40">{text('Tillämpa', 'Apply')}</button>
