...
export async function POST(request: Request) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: getRecipients(),
      replyTo: email,
      subject: `[AUTORELL ${market.code}] New contact enquiry: ${subject} – ${name}`,
      text: [
        `NEW CONTACT ENQUIRY VIA AUTORELL ${market.code}`,
        `Reference: ${reference}`,
        `Market: ${market.label} (${market.domain})`,
        `Received: ${receivedAt}`,
        `Page: ${referer}`,
        '',
        `Name: ${name}`,
        ...
        <html>
          <body style="margin:0;background:#f3f2ee;color:#202124;font-family:Arial,sans-serif;">
            <div style="display:none;max-height:0;overflow:hidden;">
              ${safeName} sent a new message via ${market.domain}.
            </div>
                  ...
                  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#ffffff;border:1px solid #deddd7;border-radius:24px;overflow:hidden;">
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                          <tr>
                            <td>
                              ...
                              <div style="margin-top:8px;color:#b4d9ef;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">New contact enquiry</div>
                            </td>
                    ...
                    <tr>
                      <td style="padding:32px;">
                        ...
                        <p style="margin:10px 0 0;color:#64747b;font-size:15px;line-height:1.7;">New message from <strong style="color:#202124;">${safeName}</strong> via ${market.label}.</p>
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:26px;background:#f7f6f2;border:1px solid #e3e2dc;border-radius:16px;">
                          <tr>
                            <td style="padding:18px 20px;border-bottom:1px solid #e3e2dc;color:#7b878c;font-size:12px;width:120px;">Name</td>
                            <td style="padding:18px 20px;border-bottom:1px solid #e3e2dc;font-size:14px;font-weight:700;">${safeName}</td>
                          </tr>
                          <tr>
                            <td style="padding:18px 20px;border-bottom:1px solid #e3e2dc;color:#7b878c;font-size:12px;">Email</td>
                          ...
                          <tr>
                            <td style="padding:18px 20px;color:#7b878c;font-size:12px;">Phone</td>
                        ...
                        <div style="margin-top:24px;border-left:4px solid #b4d9ef;background:#edf6fa;border-radius:0 14px 14px 0;padding:22px 24px;">
                          <div style="color:#66808c;font-size:10px;font-weight:700;letter-spacing:1.7px;text-transform:uppercase;">Message</div>
                          <div style="margin-top:12px;font-size:15px;line-height:1.75;">${safeMessage}</div>
                        </div>

                        <a href="mailto:${safeEmail}?subject=${encodeURIComponent(`Re: ${subject}`)}" style="display:inline-block;margin-top:28px;border-radius:999px;background:#202427;color:#ffffff;text-decoration:none;padding:15px 24px;font-size:14px;font-weight:700;">Reply to ${safeName}</a>

                        <div style="margin-top:28px;padding-top:20px;border-top:1px solid #e7e5df;color:#899196;font-size:11px;line-height:1.7;">
                          Submitted from <a href="${safeReferer}" style="color:#567587;">${safeReferer}</a><br />
                          You can also reply directly to this email. Reply-To is set to the customer's email address.
                        </div>
                      </td>
                    </tr>
                  </table>
