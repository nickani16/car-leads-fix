...
export async function PATCH(request: Request) {
  ...
  if (body.action === 'reactivate_unpaid') {
    patch.status = 'active'
    patch.payment_status = 'manual_review'
    patch.manually_activated = true
    patch.payment_warning_at = null
    patch.grace_period_ends_at = null
  }
  if (body.action === 'restrict_unpaid') {
    patch.status = 'unpaid'
  ...
  const { data, error } = await auth.adminClient
    ...
    .update(patch)
    .eq('id', body.id)
    .select('user_id,plan_key,status,payment_status,manually_activated,temporary_quota,free_period_ends_at')
    .single()
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
  if (data?.user_id && (body.action === 'reactivate_unpaid' || body.manuallyActivated || body.status === 'active')) {
