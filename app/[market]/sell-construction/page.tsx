...
export async function generateMetadata({ params }: { params: Promise<{ market: string }> }): Promise<Metadata> {
  ...
  if (!locale) return {}
  return generateWhyChooseAutorellMetadataForMarket(locale, normalizeBillingMarket(market).toUpperCase(), 'construction')
}

export default async function LocalizedSellConstructionPage({ params }: { params: Promise<{ market: string }> }) {
  const { market } = await params
  const locale = resolveMarketLocale(market)
  if (!locale) notFound()
  return <WhyChooseAutorellPage vehicleKind="construction" localeOverride={locale} marketCodeOverride={normalizeBillingMarket(market).toUpperCase()} />
}
