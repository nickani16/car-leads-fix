...
export async function generateMetadata({ params }: { params: Promise<{ market: string }> }): Promise<Metadata> {
  ...
  if (!locale) return {}
  return generateWhyChooseAutorellMetadataForMarket(locale, normalizeBillingMarket(market).toUpperCase(), 'van')
}

export default async function LocalizedSellVanPage({ params }: { params: Promise<{ market: string }> }) {
  const { market } = await params
  const locale = resolveMarketLocale(market)
  if (!locale) notFound()
  return <WhyChooseAutorellPage vehicleKind="van" localeOverride={locale} marketCodeOverride={normalizeBillingMarket(market).toUpperCase()} />
}
