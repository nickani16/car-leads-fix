...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title =
    locale === 'sv'
      ...
      : locale === 'de'
        ? 'Fahrzeug in Deutschland verkaufen | Autorell'
        : translatePublic(locale, 'Sell a vehicle across Europe | Autorell')
  const description =
    locale === 'sv'
      ? 'Välj fordonskategori, skapa en annons och nå köpare i Sverige och resten av Europa.'
      : locale === 'de'
        ? 'Anzeige erstellen und Käufer in Deutschland und Europa erreichen.'
        : translatePublic(
            locale,
            'Choose a vehicle category, create a listing and reach buyers across Europe.',
          )
  const canonicalPath = headerStore.get('x-autorell-pathname') || '/salj-fordon'

  return {
    title: { absolute: cleanSeoText(title, 65) },
    description: cleanSeoText(description, 150),
    alternates: { canonical: `${host}${canonicalPath}` },
  }
}
const categories = [
